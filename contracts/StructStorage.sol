...
contract StructStorage {
    ...
    function mat(bytes memory sid, bytes32 sname, bytes32 sloc, bytes32 material, uint scon, uint spr) public {
        ...
        StructStorage.supplier memory snew = supplier(sid, sname, sloc, material, scon, spr);
        s1[sid] = snew;
        sp.push(snew);
        s++;
    }
    ...
    function pro(bytes memory mid, bytes32 mname, bytes32 mloc, bytes32 mpro, uint mcon, uint mpr) public {
        ...
        StructStorage.manufacturer memory mnew = manufacturer(mid, mname, mloc, mpro, mcon, mpr);
        m1[mid] = mnew;
        mf.push(mnew);
        m++;
    }
    ...
    function goods(bytes memory rid, bytes32 rname, bytes32 rloc, bytes32 rpro, uint rcon, uint rpr) public {
        ...
        StructStorage.retailer memory rnew = retailer(rid, rname, rloc, rpro, rcon, rpr);
        r1[rid] = rnew;
        rt.push(rnew);
        r++;
    }
